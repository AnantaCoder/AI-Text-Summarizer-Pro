    ...
    <div className="fixed right-4 top-1 flex flex-col justify-center items-center gap-2 m-1">
      ...
      >
        ...
        >
          ...
          <span className={`absolute text-xl transition-opacity duration-300 ${!isOn ? "opacity-100" : "opacity-0"}`}>
            ☀️
          </span>
        </div>
      </div>

      <span className={`text-xl font-semibold transition-all duration-500 ${isOn ? "text-indigo-500 tracking-widest" : "text-amber-500 tracking-normal"}`}>
        {isOn ? "Night" : "Day"}
      </span>
    </div>
  );
